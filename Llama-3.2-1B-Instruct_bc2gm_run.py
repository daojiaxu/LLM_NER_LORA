...
model_dir = "./llama/llama3.2-1B-Instruct"
...
if tokenizer.pad_token is None:
    tokenizer.pad_token = tokenizer.eos_token  # 设置为 eos_token 或其他你需要的 token，例如 '[PAD]'
...
def process_func(example):
    ...
    # instruction = tokenizer(
    #     f"<|begin_of_text|>{system_prompt}<|end_of_text|>\n<|begin_of_text|>{example['input']}<|end_of_text|>",
    #     add_special_tokens=False,  # 使用模型的特殊标记
    # )
...
args = TrainingArguments(
    output_dir="output/Llama-3.2-1B-Instruct_bc2gm_model",
...
swanlab_callback = SwanLabCallback(
    ...
    experiment_name="Llama-3.2-1B-Instruct-NER_bc2gm_data",
    description="使用Llama-3.2-1B-Instruct模型在bc2gm_data数据集上微调，实现关键实体识别任务。",
    config={
        "model": model_id,
        "model_dir": model_dir,
        "dataset": "_bc2gm_data",
