...
args = TrainingArguments(
    output_dir="output/Qwen2.5-7B-Instruct_bc2gm_model_r_6",
    per_device_train_batch_size=4,
    per_device_eval_batch_size=4,
    gradient_accumulation_steps=4,
    logging_steps=10,
    num_train_epochs=4,
    save_steps=250,  # 每100步保存一次checkpoint
    eval_steps=250,  # 每100步评估一次
    learning_rate=1e-4,
...
swanlab_callback = SwanLabCallback(
    ...
    experiment_name="Qwen2.5-7B-Instruct_bc2gm_data_r_6",
    description="使用Qwen2.5-7B-Instruct_r_6模型在bc2gm_data数据集上微调，实现关键实体识别任务。",
    config={
        "model": model_id,
        "model_dir": model_dir,
        "dataset": "_bc2gm_data",
